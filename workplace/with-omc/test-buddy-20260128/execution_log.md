...
| blurry_final.png | 흐린 이미지 OCR 테스트 결과 |

---

## Bug Fix: OCR Image Preprocessing

### [STEP-006] 이슈 보고: 흐린 이미지 OCR 품질 저하
- **시간**: App Execution Test 완료 후
- **주체 에이전트**: 사용자 (수동 테스트)
- **이슈 내용**: `test_sample/text_sample/img.jpg` (해리포터 책 스캔본)에서 OCR이 완전히 인식 불가능한 문자열 생성
- **이슈 예시**:
  ```
  이전: "my ak Th nr helt pap pl pe hee teint ake da dh..."
  기대: "Mr. and Mrs. Dursley, of number four, Privet Drive..."
  ```
- **원인 분석**: 기존 OCR 전처리가 흐리거나 저품질 이미지에 대한 처리 부족

### [STEP-007] OCR 이미지 전처리 개선
- **시간**: STEP-006 직후
- **주체 에이전트**: MAIN (Claude Opus 4.5 - 메인 세션)
- **모델 티어**: MAIN
- **사용 스킬**: 없음 (직접 코드 수정 - 버그 수정 작업)
- **트리거**: 사용자 요청 (버그 수정)
- **입력**: 기존 modules/ocr.py, 흐린 이미지 샘플
- **판단 내용**: OpenCV 기반 고급 전처리 추가 + 스마트 선택 알고리즘 필요
- **실행 내용**:
  1. **OpenCV 지원 추가** (`HAS_CV2` 플래그)
  2. **`preprocess_image_basic()`**: PIL 전용 기본 전처리 개선
     - 이미지 업스케일링 (1500px 미만 자동 확대)
     - 대비 향상 (Contrast enhance 1.8x)
     - 이중 샤프닝 (SHARPEN 필터 2회)
     - 자동 대비 정규화 (autocontrast cutoff 2%)
  3. **`preprocess_image_advanced()`**: OpenCV 고급 전처리 (신규)
     - 이미지 업스케일링 (INTER_CUBIC 보간)
     - fastNlMeansDenoising 노이즈 제거 (h=10)
     - 적응형 이진화 (ADAPTIVE_THRESH_GAUSSIAN_C, blockSize=31, C=10)
     - 형태학적 정리 (MORPH_CLOSE + MORPH_OPEN)
  4. **`_count_valid_words()`**: 품질 추정 함수 (신규)
     - 영어: 모음 포함 단어 수 카운트
     - 한글: 한글 문자 포함 단어 수 카운트
  5. **`extract_text()` 개선**:
     - 기본/고급 전처리 모두 실행
     - 더 많은 유효 단어 인식한 결과 자동 선택
     - Tesseract PSM 6 (책 페이지 텍스트 블록) 설정
- **출력 결과**:
  - `modules/ocr.py`: 21줄 → 110줄 (89줄 추가)
  - OpenCV 의존성: `pip install opencv-python` (이미 설치됨)
- **비고**: 직접 코드 수정 (버그 수정은 에이전트 위임 불필요)

### [STEP-008] OCR 개선 검증
- **시간**: STEP-007 직후
- **주체 에이전트**: MAIN (오케스트레이터)
- **모델 티어**: MAIN
- **사용 스킬**: 없음 (Playwright 테스트)
- **트리거**: 수정 후 검증
- **입력**: 흐린 이미지 (`test_sample/text_sample/img.jpg`)
- **실행 내용**:
  1. Python 직접 테스트 (PIL + OCR 모듈)
  2. Playwright 앱 테스트 (전체 흐름)
- **출력 결과**:
  ```
  === 개선 전 ===
  "my ak Th nr helt pap pl pe hee teint ake da dh..."

  === 개선 후 ===
  "CHAPTER ONT ... They datat think they coull hear itif anyone fouead out sbout the..."
  "Mrs. Potter was Mrs. Dursley's sister, but they hadn't met..."
  "The Dursleys knew that the Potters had a small son..."
  ```
- **검증 결과**:
  - 이전: 완전히 인식 불가 → 이후: 읽을 수 있는 영어 텍스트 (해리포터 1장 인식)
  - 일부 OCR 오류는 존재하나 (흐린 원본 한계), 문맥 이해 가능한 수준
